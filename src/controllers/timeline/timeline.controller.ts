...
// 引入工具函数
import { Response, ApiError, catchAsync } from "@/utils";

// 获取时间线列表
export const getTimelineList = catchAsync(async (req, res) => {
  const list = await Timeline.find().sort({ date: -1 });
  res.json(Response.success(list));
});

// 新增时间线项
export const addTimelineItem = catchAsync(async (req, res) => {
...
// 删除时间线项
export const deleteTimelineItem = catchAsync(async (req, res) => {
  const { id } = req.params;
